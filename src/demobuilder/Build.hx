...
class Build extends Command {
	...
	override public function execute():Void {
		var name:String = this.console.getOption('name', 'Please provide a name for the new demo e.g. -name awesomeDemo');
		...
		var src_folder = FileSys.createDirectory(DEMO_PATH + name);
		var bin_folder = FileSys.createDirectory(BIN_PATH + name);
		
		var new_cls = File.write(DEMO_PATH + name + massive.neko.io.File.seperator + class_name + HX, true);
		var new_cmd = File.write(DEMO_PATH + name + massive.neko.io.File.seperator + class_name + HXML, true);
